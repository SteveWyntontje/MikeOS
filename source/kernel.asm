...
;
; This is loaded from the drive by BOOTLOAD.COM, as KERNEL.COM.
...
no_change:
	...
	call os_file_exists
	jc no_autorun_com		; Skip next three lines if AUTORUN.COM doesn't exist

	mov cx, 32768			; Otherwise load the program into RAM...
	call os_load_file
	jmp execute_com_program		; ...and move on to the executing part
...
app_selector:
	...
	mov si, ax			; Did the user try to run 'KERNEL.COM'?
...
not_bas_extension:
	...
	kern_file_name		db 'KERNEL.COM', 0
	...
	kerndlg_string_2	db 'KERNEL.COM is the core of MikeOS, and', 0
	kerndlg_string_3	db 'is not a normal program.', 0

	ext_string_1		db 'Invalid filename extension! You can', 0
	ext_string_2		db 'only execute .COM or .BAS programs.', 0
